...
for iter = 1:P_scp.max_iters
    ...
    % Compute predicted merit at current reference trajectory
    % Initialize slack values for merit prediction (assume zero slacks at reference)
    pred_s_v = zeros(2, P_scp.N);
    pred_s_w = zeros(1, P_scp.N);
    pred_s_T_upper = zeros(1, P_scp.N);
    pred_s_T_lower = zeros(1, P_scp.N);
    pred_s_delta_upper = zeros(1, P_scp.N);
    pred_s_delta_lower = zeros(1, P_scp.N);
    pred_s_terminal = zeros(P_scp.n_states-1, 1);
    
    pred_merit = compute_merit(X_ref, U_ref, pred_s_v, pred_s_w, ...
                              pred_s_T_upper, pred_s_T_lower, pred_s_delta_upper, pred_s_delta_lower, pred_s_terminal, ...
                              P_scp, t_remaining, iter);
    
    % Enhanced QP solving with retry mechanism
    [z, cost_val, exitflag, retry_level] = solve_qp_with_retry(X_ref, U_ref, P_scp, t_remaining, iter);
    ...
    prob = build_subproblem_2d(X_ref, U_ref, P_scp, t_remaining, 0, iter);
    ...
    % Safety checks for merit values
    if ~isfinite(pred_merit)
        fprintf('    Warning: predicted merit is not finite (%.2e), setting to large value\n', pred_merit);
        pred_merit = 1e10;
    end
    if ~isfinite(act_merit)
        fprintf('    Warning: actual merit is not finite (%.2e), setting to large value\n', act_merit);
        act_merit = 1e10;
    end
    
    % Compute merit ratio for trust region update
    if ~isempty(prev_merit) % Skip first iteration
        denom = pred_merit - prev_merit;
        if abs(denom) > 1e-12 % Avoid division by zero
            merit_ratio = (pred_merit - act_merit) / denom;
        ...
        old_trust_T = trust_T;
        old_trust_delta = trust_delta;
        old_trust_vx = trust_vx;
        old_trust_vy = trust_vy;
        ...
        fprintf('    Trust region changes: T: %.2e→%.2e, δ: %.2e→%.2e, vₓ: %.1f→%.1f, vᵧ: %.1f→%.1f, ω: %.3f→%.3f\n', ...
            ...
            % Degenerate case: pred_merit ≈ prev_merit
            merit_ratio = NaN;
            trust_update_factor = 1.0;
            fprintf('  Degenerate merit ratio: keeping trust regions unchanged\n');
        end
    else
...
function [z, cost_val, exitflag, retry_level] = solve_qp_with_retry(X_ref, U_ref, P_scp, t_remaining, iter)

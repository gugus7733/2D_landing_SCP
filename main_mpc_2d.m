        ...
        elseif n_step_open_loop_active && n_step_remaining_steps > 0
            ...
            if open_loop_active && sim_time >= t_open_loop
                ...
                if isempty(scp_sol)
                    ...
                    final_error = scp_sol.X(:,end) - [P.x_target;P.y_target;P.vx_target;P.vy_target;P.theta_target;P.omega_target;P.m_dry];
                    fprintf('SCP converged in %d iterations, max retries = %d. Predicted pos error: %.1f, Predicted V error: %.1f\n', ...
                        length(scp_log.cost), max(scp_log.retry_level), sqrt(final_error(1)^2+final_error(2)^2), sqrt(final_error(3)^2+final_error(4)^2));
                end
